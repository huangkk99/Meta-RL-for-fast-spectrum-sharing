...
class V2Vchannels:
    ...
    def __init__(self):
        self.h_bs = 1.5  # the height of the transmitter
        self.h_ms = 1.5  # the height of the receiver
...
class V2Ichannels:
    ...
    def __init__(self):
        self.h_bs = 25  # the height of the base station
        self.h_ms = 1.5  # the height of the vehicle
...
class Vehicle:
    ...
    def change_speed(self, speed_type):
        self.velocity = np.random.randint(speed_type * 10, speed_type * 10 + 5)


# the environment of the vehicular network
class Environ:
    ...
    def __init__(self, down_lane, up_lane, left_lane, right_lane, width, height, n_veh):
        ...
        self.V2V_power_dB_List_new = [23, 15, 5, -100]
        self.sig2_dB = -114  # noise power in dB
        self.bsAntGain = 8
        ...
        self.n_RB = n_veh  # the number of the sub-band
        self.n_Veh = n_veh  # The number of vehicles in the system

        self.time_fast = 0.001  # update fast fading every 1ms
        self.time_slow = 0.1  # update slow fading/vehicle position every 100 ms
        self.bandwidth = int(1e6)  # bandwidth per RB, 1 MHz

        # Pre-define varing parameters
        self.n_neighbor = 1  # the number of V2V links that each vehicle constructs with its neighboring vehicles
        self.V2V_Interference_all = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2

        self.demand_size = int((4 * 190 + 300) * 8 * 2)  # the size of V2V payload

        self.speed = 1 # the speed of the vehicles

        self.V2I_type = 0  # the fast fading of the V2I links
        self.V2V_type = 0  # the fast fading of the V2V links
        self.V2I_k = self.V2I_type * 5 + 10  # Rician factor for V2I Links
        self.V2V_k = self.V2V_type * 2  # Rician factor for V2V Links

    def set_parameters(self, num_neighbor, V2I_type, V2V_type, speed, demand_size):
        # set the specific parameters of the environment to get a specific vehicular environment. i.e., a new task

        self.n_neighbor = num_neighbor
        self.V2V_Interference_all = np.zeros((self.n_Veh, self.n_neighbor, self.n_RB)) + self.sig2
        self.renew_neighbor()  # update the neighbor

        self.V2I_type = V2I_type
        self.V2V_type = V2V_type
        self.V2I_k = self.V2I_type * 5 + 10
        self.V2V_k = self.V2V_type * 2

        self.speed = speed
        for i in range(self.n_Veh):
            self.vehicles[i].change_speed(self.speed)

        self.demand_size = int((4 * 190 + 300) * 8 * demand_size)

    def set_parameters_test(self, num_neighbor, V2I_type, V2V_type, speed):
        # set the specific parameters of the environment to get a specific vehicular environment for testing.
        ...
        self.V2I_type = V2I_type
        self.V2V_type = V2V_type
        self.V2I_k = self.V2I_type * 4 + 17
        self.V2V_k = self.V2V_type * 2 + 1

        self.speed = speed
        for i in range(self.n_Veh):
            self.vehicles[i].change_speed(self.speed)

        self.demand_size = int((4 * 190 + 300) * 8 * 2)
    ...
    def add_new_vehicles_by_number(self, n): # add new vehicles. Note: add vehicles by 4, 8, 12, ...
        ...
        for i in range(n):
            ...
            self.add_new_vehicles(start_position, start_direction, np.random.randint(self.speed * 10, self.speed * 10 + 5))
    ...
    def renew_channels_fastfading(self):
        ...
        self.V2V_channels_with_fastfading = V2V_channels_with_fastfading - 20 * np.log10(
            np.abs(math.sqrt(self.V2V_k / (self.V2V_k + 1)) + math.sqrt(1 / 2 * (self.V2V_k + 1)) * (np.random.normal(0, 1, V2V_channels_with_fastfading.shape) + 1j * np.random.normal(0, 1, V2V_channels_with_fastfading.shape))))
        ...
        self.V2I_channels_with_fastfading = V2I_channels_with_fastfading - 20 * np.log10(
            np.abs(math.sqrt(self.V2I_k / (self.V2I_k + 1)) + math.sqrt(1 / 2 * (self.V2I_k + 1)) * (np.random.normal(0, 1, V2I_channels_with_fastfading.shape) + 1j * np.random.normal(0, 1, V2I_channels_with_fastfading.shape))))
    ...
    def Compute_Performance_Reward_Train(self, actions_power):
        ...
        for i in range(len(self.vehicles)):
            for j in range(self.n_neighbor):
                if not self.active_links[i, j]:
                    continue
                V2I_Interference[actions[i][j]] += 10 ** ((self.V2V_power_dB_List_new[power_selection[i, j]] - self.V2I_channels_with_fastfading[i, actions[i, j]]
        ...
        for i in range(self.n_RB):  # scanning all bands
            ...
            for j in range(len(indexes)):
                ...
                V2V_Signal[indexes[j, 0], indexes[j, 1]] = 10 ** ((self.V2V_power_dB_List_new[power_selection[indexes[j, 0], indexes[j, 1]]]
                ...
                for k in range(j + 1, len(indexes)):  # spectrum-sharing V2Vs
                    ...
                    V2V_Interference[indexes[j, 0], indexes[j, 1]] += 10 ** ((self.V2V_power_dB_List_new[power_selection[indexes[k, 0], indexes[k, 1]]]
                                                                              - self.V2V_channels_with_fastfading[indexes[k][0]][receiver_j][i] + 2 * self.vehAntGain - self.vehNoiseFigure) / 10)
                    V2V_Interference[indexes[k, 0], indexes[k, 1]] += 10 ** ((self.V2V_power_dB_List_new[power_selection[indexes[j, 0], indexes[j, 1]]]
                ...
                for k in range(len(self.vehicles)):
                    for m in range(len(channel_selection[k, :])):
                        ...
                        if i == k and j == m or channel_selection[i, j] < 0:
                            continue
                        V2V_Interference[k, m, channel_selection[i, j]] += 10 ** ((self.V2V_power_dB_List_new[power_selection[i, j]]
