...
class Environ:
    ...
    def renew_neighbor(self):
        ...
        for i in range(len(self.vehicles)):
            ...
            for j in range(self.n_neighbor):
                self.vehicles[i].neighbors.append(sort_idx[j + 1])  # 不取自己
    ...
    def renew_channels_fastfading(self):
        ...
        self.V2V_channels_with_fastfading = V2V_channels_with_fastfading - 20 * np.log10(
            np.abs(math.sqrt(self.V2V_k / (self.V2V_k + 1)) + math.sqrt(1 / 2 * (self.V2V_k + 1)) * (
                        np.random.normal(0, 1, V2V_channels_with_fastfading.shape) + 1j * np.random.normal(0, 1,
                                                                                                           V2V_channels_with_fastfading.shape))))
        ...
        self.V2I_channels_with_fastfading = V2I_channels_with_fastfading - 20 * np.log10(
            np.abs(math.sqrt(self.V2I_k / (self.V2I_k + 1)) + math.sqrt(1 / 2 * (self.V2I_k + 1)) * (
                        np.random.normal(0, 1, V2I_channels_with_fastfading.shape) + 1j * np.random.normal(0, 1,
                                                                                                           V2I_channels_with_fastfading.shape))))
    ...
    def Compute_Performance_Reward_Test_rand(self, actions_power):
        ...
        reward_elements = V2V_Rate / 10
        reward_elements[self.demand_rand <= 0] = 1
    ...
    def act_for_testing_rand(self, actions):
        ...
        action_temp = actions.copy()
        V2I_Rate, V2V_Rate, reward_elements = self.Compute_Performance_Reward_Test_rand(action_temp)
        ...
        return reward  # V2I_Rate, V2V_success, V2V_Rate
